...
class SqlExecutor:
    ...
    def _register_udfs(conn: duckdb.DuckDBPyConnection) -> None:
        ...
        def _try_parse(phone_str: str):
            """Parse a phone number, stripping up to 4 trailing zeros to fix
            Excel floating-point artifacts (e.g. 7894111434.0000 → 78941114340000)."""
        ...
        def get_phone_type(phone_str: str) -> str:
            if not phone_str:
                return None
            p = _try_parse(phone_str)
            if p is None:
        ...
        conn.create_function('get_phone_type', get_phone_type, [VARCHAR], VARCHAR, null_handling='special')
