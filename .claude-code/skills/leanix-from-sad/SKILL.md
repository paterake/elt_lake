...
- User asks to create or update an "overview", "summary", or "all integrations" diagram
- User provides individual integration XML files as input

### Input

- **Individual integration XML files** — one or more `.xml` files previously generated by this skill
- **Existing overview XML** (optional) — if updating, the user provides the path to the current overview

### Output Location

- If creating new: save to the path specified by the user (default: `~/Downloads/`)
...
The overview notes consolidate information across all integrations. These form the **superset** — include all that are relevant, populated from the individual integration XMLs.

#### Core Sections (Always Include)

**1. Data Protection**
- Summary of encryption standards across all integrations
- PGP encryption details (which integrations use it)
- SSH key authentication summary

**2. Integration Protocols**
- Protocol per integration (HTTPS REST, SFTP, ISO 20022, etc.)
- Integration mechanism per integration (EIB, Cloud Connect, Document Transformation)
- Summary of Workday connector types used

**3. Legal Entity Segregation**
- Per-legal-entity breakdown: which integrations serve which entity
- THE FOOTBALL ASSOCIATION LIMITED integrations
- WEMBLEY NATIONAL STADIUM LTD integrations
...
- ENCRYPTION STANDARDS: at rest, in transit, PGP (per integration)
- AUTHENTICATION METHODS: per integration
- COMPLIANCE CERTIFICATIONS: per vendor/platform
- DATA RESIDENCY: per platform
- ACCESS CONTROLS: ISU, ISSG, UBSG patterns
- AUDIT & MONITORING: retention, process monitor, archival
...
- Data extracted from individual diagram notes

#### Conditional Sections (Include When Relevant)

**7. Costings Summary** — when any integration has costing data

**8. Environment Strategy** — when multiple environments (Sandbox/QA/Production) need documenting

**9. Critical Constraints** — when any integration has constraints
...
When reading individual integration XML files to build the overview, extract:
