...
```bash
# Interactive mode
elt-llm-query --config examples/dama_only.yaml

# Single query
elt-llm-query --config examples/dama_only.yaml --query "What is data governance?"
```
...
elt-llm-query --config examples/dama_fa_combined.yaml --query "How does data governance relate to financial controls?"

# Query all collections
elt-llm-query --config examples/all_collections.yaml --query "..."
```

### Override Collection

```bash
# Query a specific collection regardless of config
elt-llm-query --config examples/dama_only.yaml --collection fa_handbook --query "..."
```
...
```bash
elt-llm-query --config examples/my_custom.yaml --query "..."
```
...
│   ├── fa_handbook_only.yaml    # Query FA Handbook only
